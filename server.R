...
function(input, output, session) {
  ...
  output$plot0 = renderPlotly({
    ...
    if(input$PlotCombine=="Yes"){
      p=plot_ly(data = Comb, x=~time, y=~value, color=~variableLegend,line = list( width = 4), type='scatter', mode='lines')
    }else{
      if(FALSE){
        p=plot_ly(data = out, x=~time, y=~value, color=~variableLegend,line = list( width = 4), type='scatter', mode='lines')
      }else{
        #don't want to show the I0 class in the plot
        outSym=out[out$variable!="I0",]
        p=plot_ly(data = outSym, x=~time, y=~value, color=~variableLegend,line = list( width = 4), type='scatter', mode='lines')
      }
    }
    
    p=layout(p,xaxis=list(title="Time (days)"),yaxis=list(title=paste("Number per",formatC(N,big.mark=",",format="f",digits=0),"people"),type=input$yscale),
             annotations=list(text=HTML(paste("R", tags$sub(0),'=',formatC(Ro,digits=2))),
